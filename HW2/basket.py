...
def basket_price_mc(
    ...
    if( bsm ) :
        ...
        '''
        prices = forward[:,None] * np.exp(
                                            (-0.5 * np.diag(cov_m) * texp)[:, None]
...
def basket_price_norm_analytic(
    ...
    # _spot = spot @ weights
    # sigma = vol
    # cov_m = cor_m * sigma * sigma.reshape(-1, 1)
    # _sigma = np.sqrt(weights @ cov_m @ weights)

    # norm1 = pf.Norm(sigma=_sigma, intr=intr, divr=divr)
    # price = norm1.price(strike, _spot, texp, cp=cp)

    div_fac = np.exp(-texp*divr)
    disc_fac = np.exp(-texp*intr)
    forward = spot / disc_fac * div_fac
    _forward = forward @ weights
    sigma = vol * spot
    cov_m = cor_m * sigma * sigma.reshape(-1, 1)
    _sigma = np.sqrt(weights @ cov_m @ weights)

    norm1 = pf.Norm(sigma=_sigma, )
    price = norm1.price(strike, _forward, texp, cp=cp)
